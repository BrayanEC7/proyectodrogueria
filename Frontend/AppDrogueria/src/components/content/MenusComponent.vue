										...
										<tbody>
											<tr v-for="itemMenu in listamenus ">
												...
												<td class="text-center">
													<i class="fa fa-pencil" v-on:click="editarMenu(itemMenu)"
														style="cursor:pointer; padding-right: 1ex; color: burlywood;"></i>
													<i class="fa fa-trash" v-on:click="eliminarMenu(itemMenu.id)"
					...
					<div class="x_content">
						...
						<form id="frmMenu" v-on:submit="guardardatos()" data-parsley-validate
							...
							<div class="item form-group">
								<label class="col-form-label col-md-3 col-sm-3 label-align" for="nombre">Nombre de Menu
							...
							<div class="item form-group">
								<label class="col-form-label col-md-3 col-sm-3 label-align" for="des">Icono <span
										class="required">*</span>
								</label>
								<div class="col-md-6 col-sm-6 ">
									<input type="text" id="icono" name="icono" v-model="icono" required
										class="form-control">
								</div>
							</div>
							<div class="item form-group">
							...
							<div class="form-group row">
								...
								<div class="col-md-6 col-sm-6 ">
									<select v-model="menuPadre" class="select2_single form-control" tabindex="-1">
										...
										<option v-for="itemMenu in listamenus " :value="itemMenu.id">{{ itemMenu.nombre }}
										</option>
									</select>
								</div>
							</div>
	...
	methods: {
		async iniciarCarga() {
			try {
				...
				if (this.searchnombre == "") urltmp = "http://localhost:8080/v1/menu/menupadres";

				const response = await fetch(urltmp)
					.then(response => response.json())
					.then(data => {
						this.listamenus = this.listMenuItems = data
						console.log(data)
					});;
			} catch (error) {
				//console.log(error);
		...
		eliminarMenu(idtmp) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					fetch("http://localhost:8080/v1/menu/" + idtmp, {
